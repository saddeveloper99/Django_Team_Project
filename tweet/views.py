...
def create_post(request):
    ...
    # 데이터 수집 및 저장
        ...
        # youtube 썸네일 주소 저장
        youtube_thumbnail = ytmusic.get_song(youtube_url_check)[
        ...
        # 게시글 저장,
        new_post = Post.objects.create(owner=owner, title=title, comment=comment,
                                       youtube_url=youtube_url_check, youtube_thumbnail=youtube_thumbnail)
...
def my_page(request, user_id):
    ...
    if request.method == "GET":
        ...
        user_post = Post.objects.filter(owner=click_user).order_by('create_at')
       
        page = request.GET.get('page')
        paginator = Paginator(user_post, 8)
        page_obj = paginator.get_page(page)
        context = {
...
def set_profile(request, user_id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            profile_image.name = 'user' + str(user.user_id) + '_' + str(
        ...
        if get_user_model().objects.filter(username=new_username).exists() and new_username != user.username:
            ...
            return render(request, 'tweet/set_profile.html', {'user': user, 'error': '이름은 공백만으로 지을 수 없습니다.'})
        else:
